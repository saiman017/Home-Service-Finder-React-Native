...
export const login = createAsyncThunk("auth/login", async ({ email, password }: { email: string; password: string }, { rejectWithValue, dispatch }) => {
  try {
    ...
    const message = error.response?.data?.data || error.message || error.toString();
    dispatch(setMessage({ data: message }));
    return rejectWithValue(message);
  }
});

export const refreshTokens = createAsyncThunk("auth/refreshTokens", async (_, { getState, rejectWithValue, dispatch }) => {
  try {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(login.fulfilled, (state, action) => {
        if (action.payload) {
          const { accessToken, refreshToken, email, role, userId, isEmailVerified, otpRequired } = action.payload;
...
export const { saveToken, logout, setLoading, setEmail, setOtpRequired, clearEmail } = authSlice.actions;
